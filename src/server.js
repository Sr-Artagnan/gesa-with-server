...
//teste
server.get("/module_4", (req, res) => {
  res.render("module_4.handlebars");
});
...
server.get("/pdf", (req, res) => {
  res.render("congratulations.handlebars");
  generatePDF();
  const generatePDF = (res) => {
    const html = fs
      .readFileSync(__dirname + "/tempaltes/certified.ejs")
      .toString();
...

{
  ...
  if (req.body.cpf == item) {
    ...
    console.log("Era pra passar pra página redirect")
    insertInto();
    }
    async function insertInto() {
      res.render("redirect.handlebars", { name: req.body.name });
      await db.run(`INSERT INTO user  (name, email, cpf, module, points_Mod1, points_Mod2, points_Mod3, points_Mod4, points_Mod5, points_Mod6) VALUES  ("${req.body.name}", "${req.body.email}", ${req.body.cpf}, 1, 0, 0, 0, 0, 0, 0 );`);
  }
})
